    ...
    for layer_count, layer in enumerate(nn_layers):
        if type(layer) is Dense:
            ...
            if layer_count == len(nn_layers) - 2:
                ...
                window.blit(output_string, (neuron_x, neuron_y))
            else:
                hidden_string = font.render("Hidden Layer", True, ViewSettings.COLOR_DODGER_BLUE)
                window.blit(hidden_string, (neuron_x, neuron_y))
...
def write_controls(window, pos_x, pos_y) -> None:
    ...
    text_lines = ["CONTROLS", "ESCAPE to quit training", "WASD to move the snake", "W - UP", "S - DOWN", "A - LEFT", "D - RIGHT"]

    for i, line in enumerate(text_lines):
        text_surface = font.render(line, True, ViewSettings.COLOR_WHITE if ViewSettings.DARK_MODE else ViewSettings.COLOR_BLACK)
