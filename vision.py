...
def find_snake_head_poz(board: List[str]) -> []:
    for i in range(len(board)):
        for j in range(len(board)):
            if board[i][j] == BoardLetters.SNAKE_HEAD:
                return [i, j]
...
def look_in_direction(board: List[str], direction: Direction) -> VisionLine:
    ...
    while board[current_block[0]][current_block[1]] != BoardLetters.WALL:
        if board[current_block[0]][current_block[1]] == BoardLetters.APPLE and apple_found == False:
            apple_distance = distance(head_position, current_block)
            apple_coord = current_block
            apple_found = True
        elif board[current_block[0]][current_block[1]] == BoardLetters.SNAKE_BODY and segment_found == False:
